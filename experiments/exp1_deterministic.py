...
def run():
    ...
    for m in m_list:
        ...
        err_mldnn.append(np.max(np.abs(y_true - m_mean)))
    ...
    plot_expectation(t_eval, y_true, mldnn_mean_best, fem_mean_best, f"MLELM ($\\hat{{m}}$={m_list[-1]})", f"fEM (N={n_list[-1]})", "Exact Analytical", "Example 4.1", "exp1_deterministic.pdf")
    plot_pareto(times_mldnn, err_mldnn, times_fem, err_fem, "exp1_pareto.pdf", labels_mldnn, labels_fem)
    print(f"Saved exp1!\n")

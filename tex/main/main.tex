...
% Add remark for why \alpha > 1/2 is needed.

\section{Preliminaries}
In this section, we introduce the fractional operators, the M\"untz--Legendre polynomials, and the concepts of a physics-informed neural network (PINN) and a functional link neural network (FLNN) as the machine learning frameworks we utilize.
...
\begin{remark}
    ...
    \begin{equation}
        \int_0^t [(t - s)^{-\alpha}]^2 \dd{s} = \frac{t^{2\alpha-1}}{(2\alpha-1)\Gamma(\alpha)^2} < \infty,
    \end{equation}
    which is precisely the condition needed for the stochastic convolution against kernel to be well defined in $\mathcal L^2(\Omega)$. This inequality resurfaces in \S\ref{sec:malliavin} to justify the regularity of the fractional stochastic operational matrices.
\end{remark}
...
\subsection{Malliavin trace correction}
\lipsum[13]
